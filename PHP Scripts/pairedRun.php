...
function GetRunData(){
    global $session_id, $dbh, $curNr, $max_nr, $curRunID, $max_gens;
    $sql = "SELECT *
    FROM run 
    ...
    if ($runData == null || count($runData) == 0 || $runData[0]["nr"] >= $max_nr){ 
...
function GetCurrentTime() {
    ...
    $milliseconds = round(($microtime - floor($microtime)) * 1000000);
    return $dateTime->format('Y-m-d H:i:s') . '.' . sprintf('%06d', $milliseconds);
}
...
if (GensCompleted() != $max_gens){
    ...
    $final = "&myTurn=true" . GetState() . "&" . GetPositions($current_ids) . "&nr=" . $curRunInfo["nr"] . "&gens=" . GensCompleted(); 
}
else{
    $final = "&myTurn=false" . GetState() . "&" . GetPositions($current_ids) . "&nr=" . $curRunInfo["nr"] . "&gens=" . GensCompleted(); 
}
echo $final;
?>
